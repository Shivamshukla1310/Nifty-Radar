    ...
    if df.empty:
        ...
        with tab3:
            ...
            actual = scaler.inverse_transform(
                input_data[100:].reshape(-1, 1)
            ).flatten()

            # Prediction graph
            comp = go.Figure()
            comp.add_trace(go.Scatter(
                y=actual, mode='lines', name='Actual', line=dict(color='green')
            ))
            comp.add_trace(go.Scatter(
                y=preds, mode='lines', name='Predicted', line=dict(color='red')
            ))
        ...
        with tab4:
            ...
            st.info(
                f"**Last predicted closing price:** {last_pred:.2f}\n\n"
                f"**Last actual closing price:** {last_actual:.2f}"
            )
